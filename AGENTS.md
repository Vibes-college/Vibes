...
- 按影响选择检查：纯治理文档运行docs:check与format:check；工具代码运行check；网页/路由/构建/测试基础设施运行verify与budget。无法判定影响时完整验证；必需检查失败不得跳过。main发布只有取得与最终完整文件树一致的可信PR验收证明时可复用回归结论，仍运行本次生产构建、基础检查、预算、产物及线上核验；无证明或有冲突自动完整回退。范围分类见docs/system/checks-and-release.md。
