...
test('dependency patches cannot change unapproved packages or compiler internals', () => {
  for (const name of [
...
test('ordered patches verify an intermediate hash and unwind the complete chain', () => {
  const f = fixture();
  try {
    const secondContent = f.content.replace('-original\n+adapted', '-adapted\n+final');
    const second: DependencyPatch = {
      path: join(f.source, 'second.patch'),
      sha256: hash(secondContent),
      files: [{ path: f.name, before: hash('adapted\n'), after: hash('final\n') }],
    };
    writeFileSync(second.path, secondContent);
    const restore = applyDependencyPatches(f.source, [f.patch, second]);
    assert.equal(readFileSync(join(f.source, f.name), 'utf8'), 'final\n');
    restore();
    assert.equal(readFileSync(join(f.source, f.name), 'utf8'), 'original\n');
    assert.throws(() => applyDependencyPatches(f.source, [second, f.patch]), /before hash/);
  } finally {
    rmSync(f.source, { recursive: true, force: true });
  }
});
