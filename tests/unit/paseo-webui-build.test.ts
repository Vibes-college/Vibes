...
test('source identity rejects wrong SHA, remote, lockfile and dirty checkout', () => {
  ...
  try {
    ...
    writeFileSync(join(f.source, 'LICENSE'), 'changed license');
    assert.throws(() => verifySource(f.source, f.identity), /license/);
    writeFileSync(join(f.source, 'LICENSE'), 'Fixture license\n');
...
test('unknown output is preserved and source validation failure invalidates a known output', () => {
  const f = fixture();
  try {
    assert.throws(() => buildWebUI({ ...f, output: f.source, patches: [] }), /separate/);
    assert.throws(() => buildWebUI({ ...f, output: f.root, patches: [] }), /separate/);
    mkdirSync(f.output);
...
test('source refuses borrowed Git storage and symlinked checkouts without modifying them', () => {
  const f = fixture();
  try {
    const link = join(f.root, 'source-link');
    symlinkSync(f.source, link);
    assert.throws(() => verifySource(link, f.identity), /symbolic link/);
    const alternates = join(f.source, '.git/objects/info/alternates');
    writeFileSync(alternates, '/borrowed/objects\n');
    assert.throws(() => verifySource(f.source, f.identity), /independent Git storage/);
    assert.equal(readFileSync(alternates, 'utf8'), '/borrowed/objects\n');
    rmSync(alternates);
    verifySource(f.source, f.identity);
  } finally {
    rmSync(f.root, { recursive: true, force: true });
