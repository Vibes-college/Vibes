...
原生overlay支持按step id的replace/remove，不支持新增workflow输入。完整授权的speckit流程用overlay删除两处固定gate；阶段审阅用单阶段Skill并遵守用户暂停点，不用自动完整workflow。上游gate定义仍保留，不用文本匹配用户输入来猜测是否已批准，也不伪造审批结果。

## 依据与验证

依据本机Spec Kit 1.0.4 CLI帮助及presets/**init**.py、presets/_commands.py、workflows/overlays/schema.py、workflows/steps/gate/**init**.py源码。使用同版本原生解析和隔离生成验证，不启动模型或执行用户项目代码。OpenAI文章提供优化原则，具体机制以本机工具为准。
