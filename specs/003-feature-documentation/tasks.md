...
验收：2026-09-06实际工作区npm run check通过（类型、lint、格式、文档对应检查及45项单元测试）；临时Git用例验证源码漂移和未覆盖新增代码失败、complete基线冻结、只读摘要不修改文档。当前文档无失效本地链接，指定文件已移除，001/002历史正文未改；产品源码相对be9dd7a无变化，保留此前仍有效的产品验收，不重复声称发布。

补验：索引排序不得原地修改feature-ids，否则同一冻结文件会误报；冻结基线取比较提交与main的共同祖先，未合并的complete稿保持可修订。纯函数与真实分支→合并CLI回归覆盖这两个边界。
