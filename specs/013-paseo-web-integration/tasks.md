...
- [x] T034 [P] 在原生src/embedded/相关单测覆盖首次默认预设与后续项目选择，移除Chat/Build可见模式及双模式状态；compact从当前工作区选择原生最近会话或草稿，保留打开的文件/终端，不重建history或覆盖已有模型、草稿与项目。原生新草稿的Codex/Luna预设已由用户取消，后续默认目录与原生模型选择调整由T018复核。
...
- [x] T042 修复最终CI暴露的图片hover测试进入边界与既有媒体跳转取消竞态；保留所有断言，验证连续迟到播放、静稳后原生播放路径和显式新播放不受旧计时器影响，并在阅读路径及规则中说明250毫秒兼容保护的局限。
...
2026-09-12产品提交`010f66fedade48e778357e654c2195e9f94c83e3`正常执行`release:preview 12`退出码0：check含121项单测及类型/lint/格式/文档全部通过，三浏览器E2E 244项通过、5项条件跳过、0失败，budget通过。5项跳过仍是设备不适用的触摸/移动可见视口及非桌面内容生命周期测试。原生补丁构建20个测试文件159项与app类型检查通过，原生资源为`/vendor/paseo/61a852015cc19b54/`。
...
T042局部证据：2026-09-12最终三浏览器专项12项通过，包含真实原生音频暂停/恢复、视频跳转取消后沿原生play事件恢复、150/330毫秒两次真实播放事件被暂停以及显式播放废弃旧计时器；图片仍检查实际解码、重新进入后opacity为1及点击关闭。原始记录为`converge/seek-guard-final-e2e.log`，CI首次完整结果242通过、5跳过、2失败与trace保存在`converge/ci-ed726ac-verify.log`和`converge/ci-ed726ac-failed/`。250毫秒连续静稳为有界兼容处理，不是任意延迟恢复保证；保护期内原生播放可能需再按一次，页面按钮立即恢复。后续完整检查、最终SHA独立复核及更新预览以PR验收记录为准，不将此专项合计冒充整轮通过。

T042验证边界补充：按用户2026-09-12授权和[R9](research.md#r9-按实际影响收窄ci阻断范围)，文章引用测试不重复独立键盘遍历；合成连续play压力测试的按钮显示改为诊断，实际暂停与恢复断言保留。产品代码保持610afee，额外进度补偿未纳入；610afee本地整轮247通过、5条件跳过、budget及同SHA预览通过，云端245通过、5条件跳过、上述2项失败的原始记录保留。调整后的三浏览器专项12项通过（含独立键盘用例，1.8分钟），记录为`converge/ci-scope-adjusted-e2e.log`；最终必需CI以PR工作台为准。
...
快速收尾先落实T018目录/原生选择与测试收尾修复，再完成T022/T031当前范围验收、T030现状说明与T032独立审查；T041随同一提交验证。下列延期项只记录真实未完成范围，不打完成勾。
...
用户于2026-09-11要求宿主漏接项只记录、保持现有预览状态。以下原任务保留编号和内容作为后续依据，未实施/未验收；它们不计入当前快速收尾完成清单，不代表已通过。完整插件加载与任意直连/relay兼容矩阵同样延期。后续开始开发时另立修订规格，不能修改已经合并冻结的正文。
