...
2026-09-12用户授权对不严重的CI失败调整或取消检查，避免为低影响失败反复扩大实现与审查。610afee云端失败中，文章fixture在Astro启用入口前focus，属于测试准备时序；独立键盘用例已经覆盖气泡重新显示与遍历，引用用例等待enabled后只验证草稿/附件业务。

媒体剩余失败发生在两次人为element.play注入后：已恢复真实播放、进度前进，但WebKit按钮通知停在loading。按影响判定为该合成场景的显示诊断，保留paused/currentTime/pressed/loading证据，不再以最后一项按钮同步阻断。连续异常恢复时必须真正触发play且被暂停、之后实际恢复，以及普通原生按钮同步仍是必需断言；取消跳转后的非预期播放保护保留。未提交的额外播放器进度补偿与缺失通知模拟撤回，不声称此低优先级显示异常已修复。
