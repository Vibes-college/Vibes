...
A4已完成九语言按需与英文回退的工程探针、原生类型检查和专项覆盖；T031的B中英排除清单仍未完成，因此不勾选。T032前已确认客户端插件与现有生产CSP冲突，接入范围待用户决定，见[research R22](research.md#r22原生插件执行与网站csp的实际冲突)。
...
- [x] T007 在scripts/paseo-webui-experiment.ts与tests/fixtures/paseo-webui/冻结B0/H/A1—A6/A/B/P配置、W0—W5输入及哈希、冷暖缓存、版本和采样规则，输出审核资源上限与基线摘要到third_party/paseo-webui/budget-baseline.json，原始证据存resources/evidence/012-paseo-webui-loading/baseline/；验证无结果数据不得生成通过结论。
