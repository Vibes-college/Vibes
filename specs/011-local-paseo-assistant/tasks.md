...
既有通过项保留其原环境和范围，不当作Safari长期恢复已通过。用户已确认research的恢复、诊断与验收方案并要求继续实现；US4/FR008的真机恢复问题尚未解决，规格保持in-progress。
...
- [x] T021 评估源码移植与重写边界、Cindy/Lody采用时机、本地可观测性、成本约束及分层E2E/真机矩阵，写入research；不把方案评估当作实现或验证通过。

## 已批准恢复方案的实现

- [x] T022 补恢复/诊断/操作未知结果单元回归与故障夹具，保留旧逻辑来源和测试对应，不把新适配称为原样复用。
- [x] T023 实现单连接恢复所有权、前后台活动与有界探活/重建、canonical订阅/历史重试和旧快照门控，保留官方SDK/UI。
- [x] T024 实现白名单本地诊断、阶段状态与主动导出、跨刷新未知操作核对，验证容量/清理/去秘密。
- [ ] T025 扩展Playwright故障时序与恢复资源回归，同步功能/系统说明、规格及源码对应，运行verify与budget。
- [ ] T026 保存推送、发布同SHA预览，真实daemon与内置浏览器验收，准备并执行可获得的iPhone矩阵；不可自动操作的真机步骤保留待测。
- [ ] T027 独立Agent审阅整个PR并复核最终SHA，修复发现的问题；真机恢复未通过则保持Draft并明确剩余项。
...
- [ ] T028 根据真实Luna重任务验收修正停止边界、缺轮次的迟到工具结果和命令退出码提示；补单元/浏览器回归，独立复核及最终预览复验。
