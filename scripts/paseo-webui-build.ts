...
export function verifySource(source: string, identity: UpstreamSource) {
  ...
  const control = join(source, '.git');
  if (
    !lstatSync(control).isDirectory() ||
    lstatSync(control).isSymbolicLink() ||
    existsSync(join(control, 'commondir')) ||
    existsSync(join(control, 'objects/info/alternates'))
  )
    throw new Error(
      'Source must have independent Git storage, not a worktree or borrowed object store.',
    );
...
export function buildWebUI(options: {
  ...
  if (
    absoluteOutput === resolve(source) ||
    absoluteOutput.startsWith(resolve(source) + sep) ||
    resolve(source).startsWith(absoluteOutput + sep)
  )
    throw new Error('Output must be separate from the upstream checkout.');
      ...
      JSON.stringify(
        {
          ...
          files: [
            ...files,
            ...['PASEO-LICENSE', 'THIRD_PARTY_NOTICES.json'].map((path) => {
              const data = readFileSync(join(staging, path));
...
function main() {
  ...
  if (action === 'fetch' || action === 'ci' || action === 'production') {
    ...
    verifySource(source, identity);
    if (action === 'fetch') {
      console.log('Fixed source verified. This command does not install dependencies.');
      return;
    }
    execFileSync(
      'npm',
      [
        ...
        ...identity.workspaces.map((name) => `--workspace=${name}`),
      ],
      { cwd: source, stdio: 'inherit' },
    );
    // Apply the fixed upstream's package patches explicitly; do not run root prepare,
    // Electron downloads, browser installs or unrelated native package lifecycle scripts.
    execFileSync(process.execPath, ['scripts/postinstall-patches.mjs'], {
      cwd: source,
      env: { ...process.env, PATH: join(source, 'node_modules/.bin') + ':' + process.env.PATH },
      stdio: 'inherit',
    });
    verifySource(source, identity);
  }
  if (action === 'ci' || action === 'test-prepare') {
    verifySource(source, identity);
    execFileSync('npm', ['run', 'build:server'], { cwd: source, stdio: 'inherit' });
    verifySource(source, identity);
    if (action === 'test-prepare') return;
  }
  const patches: { source: SourcePatch[]; dependencies: DependencyPatch[] } = JSON.parse(
    readFileSync(join(root, 'third_party/paseo-webui/patches/series.json'), 'utf8'),
  );
  const publicPath =
    '/vendor/paseo/' +
    sha256(Buffer.from(JSON.stringify({ source: identity.commit, patches }))).slice(0, 16);
  const exportDirectory = join(root, '.scratch/paseo-webui/export');
  const receipt = buildWebUI({
    source,
    identity,
    patches: patches.source.map((patch) => ({ ...patch, path: resolve(root, patch.path) })),
    dependencyPatches: patches.dependencies.map((patch) => ({
      ...patch,
      path: resolve(root, patch.path),
    })),
    output: join(root, '.scratch/paseo-webui/artifacts/product'),
    exportDirectory,
    publicPath,
    mermaidSandboxSource: 'packages/app/src/components/markdown/fence/mermaid/runtime/html.gen.ts',
    exportWeb: () => {
      ...
      env.EXPO_PUBLIC_VIBES_HTML_PREVIEW_CARRIER = '/paseo-preview/';
      ...
      execFileSync(
        'npm',
        ['run', 'build:web', '--workspace=@getpaseo/app', '--', '--output-dir', exportDirectory],
        {
          cwd: source,
          env,
          stdio: 'inherit',
      ...
      execFileSync(
        ...
        [
          ...
          'unit',
          'src/embedded',
          'src/stores/draft-store/state.test.ts',
          'src/composer/actions.test.ts',
          'src/composer/input/compact-voice.test.tsx',
          'src/composer/input/state.test.ts',
          'src/hooks/use-dictation.test.tsx',
          'src/appearance/provider.test.tsx',
          'src/styles/unistyles.test.ts',
          'src/file-pane/html-preview.web.test.tsx',
          'src/file-pane/html-preview-csp.test.ts',
        ],
        { cwd: source, env: { ...env, NODE_ENV: 'test' }, stdio: 'inherit' },
      );
  ...
  console.log(`Verified product export: ${receipt.files.length} files at ${publicPath}.`);
}
