import { paseoPreviewHtml } from './paseo-webui-preview.ts';
...
export function scriptBudget(
  ...
  return {
    ...
    assistantTotalJavascriptGzip:
      size([...assistantFiles, ...nativePaths].map((path) => scripts.get(path)!)) +
      size(assistant?.previewScripts ?? []),
...
export function measureScriptBudget(out: string) {
  ...
  function scan(directory: string, relative = '') {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      else if (name === '/paseo-preview/index.html') {
        const html = readFileSync(path, 'utf8');
        if (!build || html !== paseoPreviewHtml())
          throw new Error('Unknown or changed assistant preview carrier.');
        previewScripts.push(...inlineScripts(html));
      } else if (name.endsWith('.html') && !name.startsWith('/vendor/paseo/'))
        pages.push(readFileSync(path, 'utf8'));
    }
  }
  scan(out);
  if (build && previewScripts.length !== 1)
    throw new Error('Missing verified assistant preview carrier.');
