      ...
      () => {
        ...
        if (item.kind === 'embed') {
          ...
          if (url === '/media/2048/game.txt') {
            ...
            void fetch(url, { signal: active.signal })
              .then(async (response) => {
                if (!response.ok) throw new Error('Game unavailable');
                const html = await response.text();
                if (html.length > 20000) throw new Error('Game template too large');
                if (!ready()) return;
                iframe.srcdoc = html.replaceAll(
                  '="/media/2048/',
                  `="${location.origin}/media/2048/`,
                );
                mount.append(iframe);
                show();
              })
