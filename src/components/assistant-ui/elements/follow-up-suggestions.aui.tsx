...
const FollowupSuggestionsRow: FC = () => {
  ...
  return (
    <div
      ...
      className="aui-thread-followup-suggestions aui:-my-1 aui:w-full aui:overflow-x-auto aui:py-1 aui:[-ms-overflow-style:none] aui:[scrollbar-width:none] aui:[&::-webkit-scrollbar]:hidden"
      style={{ maskImage, WebkitMaskImage: maskImage }}
    >
      <div className="aui:mx-auto aui:flex aui:min-h-8 aui:w-max aui:items-center aui:gap-2 aui:px-0.5">
        {suggestions.map((suggestion, idx) => (
          <ThreadPrimitive.Suggestion
            key={idx}
            className="aui-thread-followup-suggestion aui:bg-background aui:hover:bg-muted/80 aui:rounded-full aui:border aui:px-3 aui:py-1 aui:text-sm aui:whitespace-nowrap aui:transition-colors aui:ease-in"
            prompt={suggestion.prompt}
            send
          >
            {suggestion.title ?? suggestion.prompt}
            {suggestion.label && (
              <span className="aui-thread-followup-suggestion-label aui:text-muted-foreground aui:ms-1">
                {suggestion.label}
              </span>
            )}
          </ThreadPrimitive.Suggestion>
