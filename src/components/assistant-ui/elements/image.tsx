...
const imageVariants = cva('aui-image-root aui:relative aui:overflow-hidden aui:rounded-lg', {
  variants: {
    variant: {
      outline: 'aui:border-border aui:border',
      ghost: 'aui:',
      muted: 'aui:bg-muted/50',
    },
    size: {
      sm: 'aui:max-w-64',
      default: 'aui:max-w-96',
      lg: 'aui:max-w-[512px]',
      full: 'aui:w-full',
    },
  },
...
function ImagePreview({
  ...
  return (
    <div data-slot="image-preview" className={cn('aui:relative aui:min-h-32', containerClassName)}>
      {!loaded && !error && (
        <div
          data-slot="image-preview-loading"
          className="aui:bg-muted/50 aui:absolute aui:inset-0 aui:flex aui:items-center aui:justify-center"
        >
          <ImageIcon className="aui:text-muted-foreground aui:size-8 aui:animate-pulse" />
        </div>
      )}
      {error ? (
        <div
          data-slot="image-preview-error"
          className="aui:bg-muted/50 aui:flex aui:min-h-32 aui:items-center aui:justify-center aui:p-4"
        >
          <ImageOffIcon className="aui:text-muted-foreground aui:size-8" />
        </div>
      ) : (
        <img
          ref={imgRef}
          ...
          alt={alt}
          className={cn(
            'aui:block aui:h-auto aui:w-full aui:object-contain',
            !loaded && 'aui:invisible',
            className,
          )}
          onLoad={(e) => {
...
function ImageFilename({ className, children, ...props }: React.ComponentProps<'span'>) {
  ...
  return (
    <span
      ...
      className={cn(
        'aui:text-muted-foreground aui:block aui:truncate aui:px-2 aui:py-1.5 aui:text-xs',
        className,
      )}
      {...props}
    >
  ...
  return (
    <>
      <div
        ...
        role="button"
        tabIndex={0}
        className="aui-image-zoom-trigger aui:cursor-zoom-in"
      ...
      {isOpen &&
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            className="aui-image-zoom-overlay aui:fade-in aui:animate-in aui:fixed aui:inset-0 aui:z-50 aui:flex aui:items-center aui:justify-center aui:bg-black/80 aui:duration-200"
            onClick={handleClose}
          ...
          >
            <img
              ...
              src={src}
              alt={alt}
              className="aui-image-zoom-content aui:fade-in aui:zoom-in-95 aui:animate-in aui:max-h-[90vh] aui:max-w-[90vw] aui:cursor-zoom-out aui:object-contain aui:duration-200"
              onClick={(e) => {
                e.stopPropagation();
                handleClose();
              }}
            ...
            <button
              ...
              className="aui:text-muted-foreground aui:hover:text-foreground aui:bg-background/80 aui:absolute aui:end-4 aui:top-4 aui:cursor-pointer aui:rounded-md aui:p-2"
            >
              <XIcon className="aui:size-5" />
            </button>
          </div>,
          document.body,
        )}
...
function ImageGenerating({ className }: { className?: string }) {
  return (
    <div
      ...
      className={cn(
        'aui:bg-muted/50 aui:flex aui:min-h-32 aui:items-center aui:justify-center aui:p-4',
        className,
      )}
    >
      <Loader2Icon className="aui:text-muted-foreground aui:size-8 aui:animate-spin" />
      <span className="aui:sr-only">Generating image…</span>
    </div>
  );
}
...
function ImageContentFilterError({ className, reason }: { className?: string; reason?: string }) {
  return (
    <div
      ...
      className={cn(
        'aui:bg-muted/50 aui:flex aui:min-h-32 aui:flex-col aui:items-center aui:justify-center aui:gap-2 aui:p-4 aui:text-center',
        className,
      )}
    >
      <ShieldAlertIcon className="aui:text-muted-foreground aui:size-8" />
      <p className="aui:text-sm aui:font-medium">Image could not be generated</p>
      {reason && <p className="aui:text-muted-foreground aui:text-xs">{reason}</p>}
    </div>
  );
}
    ...
    <button
      ...
      onClick={async () => {
        ...
        try {
          ...
          // Regeneration errors are owned by the supplied adapter.
        } finally {
      ...
      className="aui:hover:bg-muted aui:inline-flex aui:size-7 aui:items-center aui:justify-center aui:rounded aui:disabled:opacity-50"
    >
      <RefreshCwIcon className={cn('aui:size-4', isRegenerating && 'aui:animate-spin')} />
    </button>
  );
}
...
function ImageActions({ part, onRegenerate, className }: ImageActionsProps) {
  return (
    <div
      ...
      className={cn('aui:flex aui:items-center aui:gap-1 aui:p-1', className)}
    >
      <button
        type="button"
        ...
        className="aui:hover:bg-muted aui:inline-flex aui:size-7 aui:items-center aui:justify-center aui:rounded"
      >
