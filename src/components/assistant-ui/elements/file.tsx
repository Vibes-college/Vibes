...
const fileVariants = cva(
  'aui-file-root aui:inline-flex aui:items-center aui:gap-3 aui:rounded-lg aui:transition-colors',
  {
    variants: {
      variant: {
        outline: 'aui:border-border aui:hover:bg-muted/50 aui:border',
        ghost: 'aui:hover:bg-muted/50',
        muted: 'aui:bg-muted/50 aui:hover:bg-muted/70',
      },
      size: {
        sm: 'aui:px-2.5 aui:py-1.5 aui:text-xs',
        default: 'aui:px-3 aui:py-2 aui:text-sm',
        lg: 'aui:px-4 aui:py-3 aui:text-base',
      },
    },
    defaultVariants: {
      variant: 'outline',
...
function FileIconDisplay({ mimeType, className, children, ...props }: FileIconDisplayProps) {
  ...
  return (
    <span
      data-slot="file-icon"
      className={cn('aui:text-muted-foreground aui:shrink-0', className)}
      {...props}
    >
      {children ?? <IconComponent className="aui:size-5" />}
    </span>
  );
}
...
function FileName({ className, children, ...props }: React.ComponentProps<'span'>) {
  return (
    <span
      data-slot="file-name"
      className={cn('aui:min-w-0 aui:flex-1 aui:truncate aui:font-medium', className)}
      {...props}
    >
...
function FileDownload({
  ...
  return (
    <a
      ...
      className={cn(
        'aui:text-muted-foreground aui:hover:bg-accent aui:hover:text-accent-foreground aui:shrink-0 aui:rounded-md aui:p-1 aui:transition-colors',
        className,
      )}
      {...props}
    >
      {children || <DownloadIcon className="aui:size-4" />}
    </a>
  );
}
...
const FileImpl: FileMessagePartComponent = ({ filename, data, mimeType, sourceType }) => {
  ...
  return (
    <FileRoot>
      ...
      <div className="aui:flex aui:min-w-0 aui:flex-1 aui:flex-col aui:gap-0.5">
        <FileName>{filename}</FileName>
        {showSize && <FileSize bytes={getBase64Size(data)} className="aui:text-xs" />}
      </div>
      <FileDownload
        data={data}
        mimeType={mimeType}
