...
从用户输入与现有上下文明确用户、核心路径、明确不做和关键边界。范围明确且用户已授权实施时，持续完成实现、必要验证与文档交付；已有授权不因进入新阶段重复确认。只有缺少影响范围、成本、权限或正确性的必要决定时询问；独立问题每轮合并最多3个，依赖问题逐个问，回答不限制词数。用户要求阶段审阅或仅做某阶段时按约定暂停；建议不自动成为需求，不顺便扩展功能。
...
验证按影响：治理文档检查标签/链接/格式，工具代码运行check，正文及受限MDX按content执行check、构建、预算与文章冒烟；影响正式网站的组件、网页或共用测试/构建基础设施运行verify与budget；纯Agent指导Markdown和已知Spec Kit配置按docs轻量检查，混入网页、脚本、依赖或GitHub工作流则升级；未知先查证并说明受影响范围，必需检查失败不得跳过。失败只暂停依赖步骤，Agent修复本任务造成的问题并重跑受影响检查；只有缺少必要信息、权限或外部条件时等待用户。main发布可复用与最终完整文件树一致的可信PR完整验收；必须核对来源、最新运行与当前重跑身份，并继续本次生产构建、基础检查、预算、产物和线上核验，证据不足或冲突自动完整回退。本地与PR按各自实际差异及依赖边界选择检查。自动化E2E本地与CI统一Playwright；内置浏览器按需用于体验和视觉审阅；实现、测试、部署分别据实报告。需要Spec Kit的需求使用分支与PR，以完整需求为审阅单位；小修复、文档和小型规则补充直接在当前工作分支提交，不单独开PR。必要功能说明、源码对应和实现完成状态随代码交付；只有可选日期或纯措辞可批量补记，不能延后现状同步。未经用户明确指令不得合并PR；小改动完成必要验证后可直接提交推送，远端保护阻止时保留进度并报告，不擅自改变保护设置或另开PR。发布核对同一SHA的检查和实际目标；用户于2026-09-06授权vibes.college为正式目标；main必要检查成功后自动发布并验收，之后才清理本目标资源。
...
当前产品仅Explore，保留UI，站内不开放公众/Agent编辑或Markdown投稿，文章改进通过GitHub PR审阅。Market、Events、Tag仅为未来方向。浏览器不得持有秘密或决定支付与权限；Worker不等于完整Node/Linux或本地Agent执行环境。
...
首版spec形成即建Draft PR，提供可打开链接和任务摘要；PR描述是当前handoff，评论留重要阶段证据，tasks为详细清单。阶段、交接、暂停或结束前提交并推送有效进度，不逐commit强制push。Draft独立轻量检查，Ready及其后更新按整个PR影响验收，分支push不再重复运行PR检查。main同次运行检查通过后发布确切SHA的产物，生产串行并拒绝过时版本；阶段预览不提升生产。部署失败不清理，成功验收后只清理本任务无额外提交、干净且无占用的分支/worktree，同步空闲的本地主目录时，有本地独有提交则保留并合并origin/main，没有则快进；未保存工作与其他任务占用须保护，不建立定时跟进。
...
**Version**: 5.1.1 | **Ratified**: 2026-09-05 | **Last Amended**: 2026-09-14
