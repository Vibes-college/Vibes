...
验证按影响：治理文档检查标签/链接/格式，工具代码运行check，正文及受限MDX按content执行check、构建、预算与文章冒烟；组件、网页及测试/构建基础设施运行verify与budget；纯Agent指导Markdown和已知Spec Kit配置按docs轻量检查，混入网页、脚本、依赖或GitHub工作流则升级；不确定时完整验证，必需检查失败不得跳过。失败只暂停依赖步骤，Agent修复本任务造成的问题并重跑受影响检查；只有缺少必要信息、权限或外部条件时等待用户。main发布可复用与最终完整文件树一致的可信PR完整验收；必须核对来源、最新运行与当前重跑身份，并继续本次生产构建、基础检查、预算、产物和线上核验，证据不足或冲突自动完整回退。本地与PR完整验收范围保持不变。自动化E2E本地与CI统一Playwright；内置浏览器按需用于体验和视觉审阅；实现、测试、部署分别据实报告。需要Spec Kit的需求使用分支与PR，以完整需求为审阅单位；小修复、文档和小型规则补充直接在当前工作分支提交，不单独开PR。必要功能说明、源码对应和实现完成状态随代码交付；只有可选日期或纯措辞可批量补记，不能延后现状同步。未经用户明确指令不得合并PR；小改动完成必要验证后可直接提交推送，远端保护阻止时保留进度并报告，不擅自改变保护设置或另开PR。发布核对同一SHA的检查和实际目标；用户于2026-09-06授权vibes.college为正式目标；main必要检查成功后自动发布并验收，之后才清理本目标资源。
...
**Version**: 5.1.0 | **Ratified**: 2026-09-05 | **Last Amended**: 2026-09-12
