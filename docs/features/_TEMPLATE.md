...
每项写成可以观察的结果。勾选表示最近一次仍有效的真实验收，注明日期、环境和方法；文档排版或新对话不清空原有证据。相关代码变化使结论失效时重新验证，未覆盖项保持未勾选。不能把旧记录写成今天重新测试。
...
顶部code-sources需列明本功能对应的源码文件或目录，code-revision在对照说明复核后填写；用npm run docs:check -- --revisions只读查看候选摘要。源文件/行为边界与正文必须一致，摘要更新不是自动审核。
