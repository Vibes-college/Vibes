...
1. 在工作分支完成一个功能、修复或一批相关维护，中途可多次commit；同一工作复用分支和已有PR。完整可审阅后再提PR，小的非紧急收尾随下一次相关工作提交；查看verify和budget结果，失败时按日志修复。
2. 同一PR准备最终功能说明及规格元数据，检查通过后由用户明确决定合并。合并后同步本地main，确认改动全部包含后清理工作分支，保留commit历史；非紧急状态与日期补记不单独催合并。
