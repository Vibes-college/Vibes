...
8. 上线验收后AI运行`npm run cleanup:task -- <PR号>`查看候选，确认无额外提交、无脏文件或其他任务占用，再执行清理；从待删除worktree之外执行，不切换其他任务的分支。收尾时将空闲且可安全快进的本地主目录同步到origin/main，不建立定时跟进。分支删除不删除Git历史或回滚版本。
