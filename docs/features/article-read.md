...
code-revision: 'f4a9cfe8e6b8f8f36237486f579cc9c3066188c58f4bf1a6ef7687be97a1fd5f'
---
...
3. 上下滑动或点轻微摆动的向下入口，概览与正文保持两整页过渡：触摸拖动跟手，松手达到翻页门槛后平滑切至另一页，否则回当前页；不会停在两页中间；正文全部展开；进入正文超过32px后关闭入口吸附，底部继续滑动保持普通滚动，回到入口附近恢复停靠。底部进度胶囊只在正文出现，显示阅读进度与当前章；点击展开目录并跳转，Escape或点外部关闭。
...
两屏过渡回归：2026-09-06，桌面Chromium原生wheel、手机Chromium原生触摸、手机WebKit DOM触摸事件覆盖轻滑回位、跟手位移、达门槛翻页、反向返回、取消及正文底部滚动。原proximity真机修复确认作为历史证据保留，但不用于证明当前整页手感；WebKit模拟也不能代替iPhone真机。当前证据在`resources/evidence/006-detail-reading/page-turn/`。
