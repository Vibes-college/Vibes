...
describes: '阅读作品详情'
status: 'current'
shaped-by: ['001', '003', '005', '006', '007', '008', '009', '010', '011']
code-sources:
  [
...
code-revision: '13d0e297aea469174e74aeb109521f963c683cddf2a55ad889c20c7460a64334'
---
...
- 视频与音频：点播放/试听后加载完整来源，可用原生播放/暂停、进度、音量与字幕控制，页面按钮同步状态；封面左上角的列表图标展开章节和文字稿，可直接跳转；跳转尚未完成时也可暂停，暂停会取消为跳转发起的补充下载，视频在章节跳转尚未完成时暂停，跳转结束后可能恢复播放，此时可再次暂停；该重叠操作不作为必须保持暂停的验收条件。结束后可用原生按钮继续播放。音频波形来自实际录音，若附文字稿可展开阅读，带时间的行可跳转；没有歌词或字幕时不会生成假内容。Sintel是完整预告片，不是整部电影；Carefree提供完整录音。
...
作品详情右下角“本地助手”可将公开作品资料交给自己的电脑Agent。点击来源标签核对后发送，完整聊天、审批、停止和恢复步骤见[使用本地助手](local-assistant.md)。助手独立滚动，关闭后继续阅读，跨站内导航保留连接。
