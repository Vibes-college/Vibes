...
code-revision: '0a912b545dac88f22ab140f9298e6011bd34edf3e4affe1cd0f168c3d191d836'
---
...
- 固定示例只有匹配当前版本的完整验收回执才显示已验证。完整test:e2e写入site-verification.json，独立great-ui:test写入standalone-verification.json；两者不互相代替，筛选重跑不签发回执。main可信复用完全相同文件树的PR回归时，重建site回执并明确引用原验收run/attempt，不声称本次重新测试。源码、Markdown、能力、规则、条件、适配器或测试变化使旧记录失效。
