...
code-revision: 'c04bf1bdce9745096706d64e8c4b885e5be379c63eb5f759b5fc6f5a1a5b52bc'
---
...
放大保留播放位置和暂停状态，窗口跨过800px时选用对应素材并按进度比例继续；手动播放不因旋转或减少动态偏好重新暂停。查看器打开时锁定背景，Tab留在内部；按Escape或“缩小录屏”退出后恢复焦点。这里使用页面内全屏，浏览器地址栏仍可能存在；不要求原生Fullscreen API。仅在调整进度（包括回到开头）或旋转恢复位置时，为当前素材读取最多2MiB原始MP4，再转换为可跳转的data URL，避免浏览器与静态服务的分段读取差异；base64编码体积约增至原始文件的4/3，解码另占浏览器内存。连续拖动复用副本并以最后一次选择为准，切换素材或离开页面会取消旧请求并释放副本。
...
- [x] 2026-09-15修复后本机媒体专项40项通过、2项按触摸注入方式跳过；Linux WebKit另有9项媒体专项通过；该专项不替代完整回归。覆盖三类两端选源、实际尺寸、放大/移动/焦点、旋转、按需请求、旧视频释放、延迟模块保留手动播放/暂停；无分段服务的连续定位、旋转取消、超限、失败不重播及卸载清理。手机Chromium含CDP双指缩放；WebKit测试控件、鼠标与键盘，不冒充真机触摸。
...
- [ ] 2026-09-20定位副本兼容修正后的完整verify、预算与产物核验待完成；2026-09-19的188项单元、365项浏览器通过及7项设备跳过保留为原版本证据。媒体专项本机46项通过、2项设备跳过，Linux15项通过、1项触摸注入条件跳过；覆盖读取取消、旧来源错误、503/超限与离页清理。证据在resources/evidence/018-great-ui-scale/release-2026-09-19。
- [x] 2026-09-19整个PR独立审查与导航修复复核通过；最终提交复核、Linux CI、合并与正式发布的状态和凭据统一在PR维护。
