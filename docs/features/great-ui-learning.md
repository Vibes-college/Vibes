...
code-revision: 'bc6ce241fa139dbd6b08d5e6fbd882024337354fad483798733246a801594183'
---
...
放大保留播放位置和暂停状态，窗口跨过800px时选用对应素材并按进度比例继续；手动播放不因旋转或减少动态偏好重新暂停。查看器打开时锁定背景，Tab留在内部；按Escape或“缩小录屏”退出后恢复焦点。这里使用页面内全屏，浏览器地址栏仍可能存在；不要求原生Fullscreen API。静态服务不支持分段读取时，仅在调整进度或旋转恢复位置时读取当前素材，最多2MiB；连续拖动以最后一次选择为准，切换素材或离开页面会取消旧请求并释放临时副本。
...
- [x] 2026-09-15媒体专项36项通过、6项按设备跳过：三类两端选源、实际尺寸、放大/移动/焦点、旋转、按需请求、旧视频释放、延迟模块保留手动播放/暂停；无分段服务的连续定位、旋转取消、超限、失败不重播及卸载清理。手机Chromium含CDP双指缩放；WebKit测试控件、鼠标与键盘，不冒充真机触摸。
- [x] 126个独立MP4全部完整解码；48桌面、30独立手机、48轻量封面的文件摘要、宽高、时长及单文件预算通过。两套构建与预算通过，普通公共/媒体/MDX脚本上限保持不变。
- [x] 2026-09-15正式站完整verify通过：340项浏览器检查通过、11项按设备跳过；生成匹配当前实现的site回执，构建、预算与Cloudflare阶段预览版本核验通过。
- [x] 整个PR实现经独立Agent审查，旋转、延迟初始化、无分段定位与卸载问题均修复并复测；最终提交复核与交付检查见PR。
