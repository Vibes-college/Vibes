...
8. 已建立连接后，页面刷新、断线重连或从后台返回会尝试恢复电脑上的历史、运行状态和待审批，再开放操作。“刷新状态”只同步现有连接，离线时需用“重新连接”；手机长时间离开后的恢复仍有用户报告失败，见已知问题。“发送结果未确认”等提示出现时，先点“刷新状态”核对历史，再决定是否重新输入；浏览器不自动重发。更早记录每次读取200条，最多显示2000条，达到上限后可返回最新记录，完整历史仍在电脑。
...
- [x] 2026-09-07用户口述手机Safari在PR预览完成首次连接、收发、切会话、工具展示及详情上下文传递；设备/系统版本未记录，非Agent重现。
- [ ] 手机Safari离开后返回恢复：用户报告“正在连接→电脑连接失败”，多次刷新状态无效；深入任务疑似浏览器`.js`工具反复失败，原文未知。断网/锁屏矩阵及真实故障复现仍待验。
