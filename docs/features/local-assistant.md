...
2. 手机或电脑打开Vibes的`/zh/`或`/en/`，点右下角“本地助手”。首次此时才下载聊天组件。把官方v2配对链接或JSON粘贴进输入框，点击“连接电脑”；有效配对通过官方中继加密连接，成功后显示“已连接”。无效配对不联网，连接失败时检查电脑和网络后重试。
...
- [ ] 审批允许、拒绝及问题回答提交真实requestId，结果未知时阻止重复操作；真实daemon与协议夹具证据分开记录。
- [ ] 桌面、移动Chromium/WebKit及320px回归通过，内置浏览器实际布局复核通过。
- [ ] 真机iPhone Safari及手机断网/锁屏后的恢复验收通过。

有效证据日期、环境和覆盖范围在验收后据实填写；自动化设备模拟不等于真机通过。
