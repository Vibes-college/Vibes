...
code-revision: 'b768c23cb7a949dd0075b26ee2df48d7f9bb887f7f3a420d5da9abf875f10e18'
---
...
Great UI学习页也使用同一作品引用入口，附上当前案例标题和本站学习网址；切换示例或复制任务不会自动发送聊天。
...
进入作品详情时，吉祥物上方显示“和Agent聊这篇”说话气泡，约5秒后消失。鼠标移到入口或用键盘聚焦时可再次显示；键盘操作期间保持可见。手机普通打开再收起助手后重新提示5秒，切到另一篇文章也会重新提示。鼠标或触摸收起时释放入口焦点；键盘或辅助技术激活收起按钮时，焦点返回刚才的入口，加载中与加载后保持一致。普通点吉祥物仍只打开助手。
...
2026-09-20收起按钮的触摸焦点释放、普通及文章入口的键盘焦点返回，在加载前后共12项三浏览器专项通过；随后完整verify通过188项单元、377项浏览器及7项设备条件跳过，预算与产物检查通过。证据在`resources/evidence/018-great-ui-scale/release-2026-09-19`；协议fixture及手机模拟不代替真实Agent会话或iPhone验收。
...
- `tests/paseo-loading.spec.ts`：普通浏览/无JS零加载、气泡5秒隐藏与鼠标/键盘重显、加载前后触屏收起释放焦点及再次提示、键盘收起返回普通或文章入口、换篇重置、即时引导、资源失败重试、单一根与动态样式保留、全屏背景锁定及浏览器前进后退的阅读位置、受控可见视口布局。
...
- `tests/paseo-csp.spec.ts`：真实图片选择、blob缩略图与灯箱解码、触屏点按及桌面鼠标重新进入图片后显示控件并关闭、带行号文件链接及HTML交互与隔离；WebKit图片用例使用独立临时持久profile并在结束时删除，其余用例保持临时上下文。fixture按构建后的响应头路径规则运行，实际Cloudflare响应仍需部署验收。
