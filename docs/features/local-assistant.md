...
5. 首次进入且没有已选工作区时，在连接的电脑准备`~/Vibes`，默认选择可用的Codex Luna及该模型原生默认思考强度。已有同名目录会保留；没有Codex/Luna或目录不可用时显示原因，可重试、选择其他Agent/项目或进入连接设置。这里只准备草稿，点击发送后才创建Agent；以后沿用Paseo当前项目和会话，不重复套用初始预设。
6. 小窗只显示对话，白色顶部一行提供新对话、展开和收起三个按钮。新对话沿用当前工作区与原生偏好；没有既有模型偏好或明确选择时，暂用可用的Codex Luna及原生默认强度，不覆盖用户选择或主动清空，填入默认值时不自动保存偏好。要选择已有项目或历史会话时，先展开，再从左侧菜单进入原生入口；点“新建工作区”进入原生创建页面。界面没有Chat/Build模式切换，已有配置、草稿和附件沿用Paseo保存。
...
- 精简页头尚未接回原生工作区专属菜单、Scripts和编辑器/Git快捷入口；原生命令中心、Changes及文本编辑器本身仍保留。设置中的插件入口仍在，但固定原生版本的插件执行需要eval，主页面没有开放，因此插件运行尚未接通。
