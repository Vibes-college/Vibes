...
Great UI在普通目录中只显示一个[交互学习合集](great-ui-learning.md)。卡片显示合集身份、48件数量和当前作品，左右箭头切换轻量预览；当前作品名称进入对应详情，合集标题进入合集。同一标签页在目录、搜索和合集首屏间保留所选封面。关键词搜索仍覆盖每件作品的说明、目标和术语，子项不重复铺进普通浏览卡片。
