...
code-revision: '6e19607f332f882527e10457fe76140aab549915c426d8ab60554901989a0251'
---
...
《Vibe Coding 网页动效与交互》在普通目录中只显示一个[学习合集](great-ui-learning.md)。卡片显示合集身份、51件数量和当前作品，左右箭头切换轻量预览；当前作品名称进入对应详情，合集标题进入活动报名卡教程，随后可沿八站路线与十类索引继续学习。学习顺序独立于封面顺序，同一标签页在目录、搜索和合集首屏间保留所选封面。关键词搜索仍覆盖每件作品的说明、目标和术语，子项不重复铺进普通浏览卡片。
...
合集仅随其所在的有限分页或搜索结果携带小型素材索引（gzip小于6KiB），不把全部51个播放器放进页面，也不预取隐藏成员海报或高清文件。可见后准备切换控件，当前短片沿用上述播放调度；切换先暂停并清除旧视频来源。延迟到达的切换模块会保留已经手动播放或暂停的首个播放器，当前操作优先于历史封面选择。首张静态海报可能在恢复所选封面前加载，其他成员海报只随选择出现；没有合集的首页不请求合集视频。
