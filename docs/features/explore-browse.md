...
合集仅随其所在的有限分页或搜索结果携带小型素材索引（gzip小于6KiB），不把48个播放器放进页面，也不预取隐藏成员海报或高清文件。可见后准备切换控件，当前短片沿用上述播放调度；切换先暂停并清除旧视频来源。延迟到达的切换模块会保留已经手动播放或暂停的首个播放器，当前操作优先于历史封面选择。首张静态海报可能在恢复所选封面前加载，其他成员海报只随选择出现；没有合集的首页不请求合集视频。
