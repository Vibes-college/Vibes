...
2026-09-06已通过GitHub API启用main保护：必须经PR、verify/budget成功且分支更新到最新main，管理员同样受限，禁止force push和删除main；未设置必须他人批准，因此用户仍可自行决定合并。
