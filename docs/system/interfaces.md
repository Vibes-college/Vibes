...
`src/features/paseo-webui/contract.ts`定义版本1的直接模块挂载接口及严格数据校验；H隔离构建已经接入Explore入口，普通构建不启用。挂载类型要求同一容器返回同一实例，真正退出后需整页重载才能重新挂载；收起和普通导航不调用退出。当前生命周期、公开资料接入、构建边界及已验证范围见[助手系统说明](local-assistant.md)。
...
draft命令更新当前可附带的候选，null清除候选，不删除用户输入。用户在原生composer明确点击后才把有标签的JSON引用放进原生草稿；草稿仍走原生发送、排队与失败恢复路径。换页不改已附带内容，也不提升资料为系统指令。
...
明确启用H时，原生Paseo可通过官方wss://relay.paseo.sh与同版隔离daemon建立加密连接，配对/会话/聊天仍由原生协议持有，Vibes未新增业务RPC。普通构建仍只允许同源连接；本地Worker已验证配对与历史恢复，完整恢复矩阵仍未完成。
