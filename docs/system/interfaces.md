...
`src/features/paseo-webui/contract.ts`定义版本1的直接模块挂载接口及严格数据校验；H隔离构建已经接入Explore入口，普通构建不启用。挂载类型要求同一容器返回同一实例，真正退出后需整页重载才能重新挂载；收起和普通导航不调用退出。当前生命周期、构建边界及已验证范围见[助手系统说明](local-assistant.md)，公开作品草稿接入仍未完成。
