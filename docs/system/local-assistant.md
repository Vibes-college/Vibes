...
换会话、连接代次改变及忘记都会使在途同步失效。只有已核对身份的事件可进入状态；快照读取时缓冲当前会话事件，安装后仅把时间线按seq归并；无可靠序号的状态/审批事件触发有界权威状态重读，完成前保持操作门控，连续变化三轮仍未收敛则退避重试。读取更早记录失败时仍消费有效实时事件，避免界面停在旧运行状态；审批变化事件立即进入恢复门控，权威重读完成前不能点击过期审批。重复seq忽略，epoch变化、缺序号或跳号重新读取tail；缓冲最多2000条，超限作废并重新补齐，不无界积压。tail是最新窗口，更早记录通过before游标读取；最多显示2000条，不承诺一次恢复下载全部历史。暂不引入旧projected投影、热会话缓存或持久正文。
