...
compact的文字发送按钮只取决于当前是否有文字；空输入始终麦克风，焦点与曾经点击不改变规则，清空立即恢复，仅有引用或其他附件也保留麦克风。该呈现直接从文字与原生录音状态派生，不另存键盘模式。录音仍由用户明确点击启动，录音发送确认继续沿原生转写、提交和排队路径携带附件；按钮恢复不代表提交成功。转写失败保留原生重试/丢弃入口，重复确认和迟到结果由原生听写状态处理。
