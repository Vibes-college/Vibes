---
tense: 'living'
describes: '项目Agent规则、Skills与workflow适配的使用和维护'
status: 'current'
shaped-by: ['015']
code-sources:
  ['.specify/presets/vibes/', '.specify/workflows/overlays/speckit/scoped-execution.yml']
code-revision: '462201aa9f034c975373bca43a60adb77149e5943505e0e18269ca908d4668f9'
---
...
`specify workflow run speckit --input spec='已授权完整实施的具体需求'`运行完整自动流程。项目overlay移除固定spec/plan双审批，不设置假审批结果；仅在已授权完整实施时使用。用户需要阶段审阅时，逐阶段调用Skill并在约定点等待，不运行这个完整自动workflow。上游workflow原件保留，实际生效层可用`specify workflow resolve speckit`查看。
...
2. `specify preset resolve speckit.plan`等命令显示当前来源；只调整文档模板则编辑`.specify/templates/overrides/`，模板不会替换Skill正文。workflow规则维护`.specify/workflows/overlays/speckit/scoped-execution.yml`。
...
4. 检查生成文件的`metadata.source`为`preset:vibes`，逐项核对内容；重复生成应得到相同结果。升级Spec Kit版本时先在隔离目录验证preset、模板和overlay解析，未知行为不要直接覆盖工作区。
...
恢复项目规则时，依据已确认的Git版本同时恢复preset源、登记、overlay及生成Skills，再核对解析结果；不要只恢复一份生成文件。若要恢复原生带审批workflow，显式禁用项目overlay并核对影响范围；日常阶段审阅优先使用单阶段Skill，避免修改其他任务共用的配置。
